...
public class StudentImpl implements IStudent {
	...
	public int add2(Integer id, String no, String name, String sex, String birth, String string, Integer id2) {
		int i = 0 ;
		try {
			conn = DB.getConn();
			pst = conn
	...
	public int s_img(String pic) {
		int i = 0 ;
		try {
			conn = DB.getConn();
			pst = conn.prepareStatement("SELECT * FROM student WHERE stu_pic like ?");
			pic = "%../images/"+pic+"%";
			pst.setString(1, pic);
			rs = pst.executeQuery();
			while (rs.next())
