	...
	<body>
		...
		<center>
			...
			<div style="width:700px;text-align: left" >	
				...
				<p>与控制。${sy.getInformtc()}</p>
			</br></div>
			</strong>
		</center>
<c:if test="${rol==2 && sy.getInformtc()!=null }">
    <script type="text/javascript">
        function showPopup() {
            alert("${sy.getInformtc()}");
        }

        setTimeout(showPopup, 1000); // 设置延迟时间为1000毫秒（1秒）
    </script>
</c:if>
<c:if test="${rol==3 && sy.getInformst()!=null }">
    <script type="text/javascript">
        function showPopup() {
            alert("${sy.getInformst()}");
        }
